...
const OAuth2RedirectHandler = () => {
  ...
  useEffect(() => {
    ...
    console.log("OAuth jwt token: ",token);
    console.log("OAuth2RedirectHandler: Params:", params.toString());
    console.log("OAuth2RedirectHandler: Token:", token);
    console.log("OAuth jwt token1a: ",token);
    if (token) {
      console.log("OAuth jwt token2: ",token);
      try {
        ...
        const user = {
          username: decodedToken.sub,//subject of the jwt
        ...
        console.log("OAuth jwt token3: ",token);
        setToken(token);
